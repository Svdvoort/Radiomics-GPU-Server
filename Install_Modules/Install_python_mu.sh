...
gcc_version=7.3.0
python_version=2.7.15MU
python_real_version=2.7.15
...
wget https://www.python.org/ftp/python/"${python_real_version}"/Python-"${python_real_version}".tgz
tar -xzf Python-"${python_real_version}".tgz
cd Python-"${python_real_version}"

# Configure with the Module folder as target folder
mkdir -p /packages/python/"${python_version}"
./configure --prefix=/packages/python/"${python_version}" --enable-optimizations --enable-unicode=ucs4
make -j2
make install
